...
function mercury_setup() {
	...
	add_image_size( 'mercury-213-120', 213, 120, true );
	...
	add_image_size( 'mercury-9999-70', 9999, 70);
...
function mercury_scripts() {
	...
	wp_enqueue_style( 'font-awesome', get_theme_file_uri( '/fontawesome/css/all.min.css' ), array(), '6.6.0' );
	...
	if( !$header_h1_title_color = get_theme_mod( 'header_h1_title_color' ) ) {
		$header_h1_title_color = '#151515';
	} else {
		$header_h1_title_color = get_theme_mod( 'header_h1_title_color' );
	}

	if( !$header_title_box_background_color = get_theme_mod( 'header_title_box_background_color' ) ) {
		$header_title_box_background_color = '#f1f3f4';
	} else {
		$header_title_box_background_color = get_theme_mod( 'header_title_box_background_color' );
	}

	if( !$header_title_box_text_color = get_theme_mod( 'header_title_box_text_color' ) ) {
		$header_title_box_text_color = '#7f8c8d';
	} else {
		$header_title_box_text_color = get_theme_mod( 'header_title_box_text_color' );
	}
...
.space-archive-title-box {
    background-color: ' . esc_attr ($header_title_box_background_color) . ' !important;
}
.space-title-box-h1 h1,
.space-archive-title-box-h1 h1 {
    color: ' . esc_attr ($header_h1_title_color) . ';
}
...
.space-archive-title-box .space-breadcrumbs a {
    color: ' . esc_attr ($header_title_box_text_color) . ' !important;
}
...
function mercury_dark_mode_css() {

	global $mercury_dark_css; 

	if( get_theme_mod('mercury_enable_dark_mode') ) {

		wp_enqueue_style( 'mercury-dark', get_theme_file_uri( '/css/dark-mode.css' ), array(), $GLOBALS['mercury_version']);
		
		
				
		// Main Background Color
				
		if( !$dark_main_background = get_theme_mod( 'dark_main_background' ) ) {
			$dark_main_background = '#121315';
		} else {
			$dark_main_background = get_theme_mod( 'dark_main_background' );
		}

		// Second Background Color
				
		if( !$dark_second_background = get_theme_mod( 'dark_second_background' ) ) {
			$dark_second_background = '#202427';
		} else {
			$dark_second_background = get_theme_mod( 'dark_second_background' );
		}

		// Third Background Color
				
		if( !$dark_third_background = get_theme_mod( 'dark_third_background' ) ) {
			$dark_third_background = '#2d3337';
		} else {
			$dark_third_background = get_theme_mod( 'dark_third_background' );
		}

		// Fourth Background Color
				
		if( !$dark_fourth_background = get_theme_mod( 'dark_fourth_background' ) ) {
			$dark_fourth_background = '#151515';
		} else {
			$dark_fourth_background = get_theme_mod( 'dark_fourth_background' );
		}

		// Titles Color
				
		if( !$dark_title_color = get_theme_mod( 'dark_title_color' ) ) {
			$dark_title_color = '#fff';
		} else {
			$dark_title_color = get_theme_mod( 'dark_title_color' );
		}

		// Second Text Color
				
		if( !$dark_second_text_color = get_theme_mod( 'dark_second_text_color' ) ) {
			$dark_second_text_color = '#c3c7c8';
		} else {
			$dark_second_text_color = get_theme_mod( 'dark_second_text_color' );
		}

		// Third Text Color
				
		if( !$dark_third_text_color = get_theme_mod( 'dark_third_text_color' ) ) {
			$dark_third_text_color = '#7f8c8d';
		} else {
			$dark_third_text_color = get_theme_mod( 'dark_third_text_color' );
		}
		...
		.space-style-3-organization-header {
			background-color: ' . esc_attr ($dark_main_background) . ';
		}
		...
		.space-header-search-block input[type="search"] {
		    border-bottom: 1px solid ' . esc_attr ($dark_main_background) . ';
		}

		.space-organization-content-rating-items,
		.space-organization-style-2-ratings-block,
		.space-unit-content-button {
		    border-top: 1px solid ' . esc_attr ($dark_main_background) . ';
		}

		.space-pros-ins:before {
		    color: ' . esc_attr ($dark_main_background) . ';
		}
		...
		span.aces-card {
		    background-color: ' . esc_attr ($dark_second_background) . ';
		}
		...
		.space-organizations-3-archive-item-provider-ins:after {
		    background-color: ' . esc_attr ($dark_third_background) . ';
		}
		...
		.space-organization-content-excerpt {
		    border-bottom: 1px solid ' . esc_attr ($dark_third_background) . ';
		}
		.space-pros-ins,
		.space-cons-ins {
		    border: 1px solid ' . esc_attr ($dark_third_background) . ';
		}
		.space-organization-details .space-organization-details-item,
		.space-dark-style .space-offers-archive-item-middle-elements-ins {
		    border-top: 1px solid ' . esc_attr ($dark_third_background) . ';
		}
		...
		.space-organization-float-bar-bg {
		    background-color: ' . esc_attr ($dark_fourth_background) . ';
		}
		.space-aces-single-offer-info-title h1 {
		    color: ' . esc_attr ($dark_fourth_background) . ';
		}
		...
		.space-organizations-8-archive-item-terms span {
		    color: ' . esc_attr ($dark_title_color) . ';
		}
		...
		.space-comments-list-item-author a {
		    color: ' . esc_attr ($dark_title_color) . ' !important;
		}
		...
		.space-footer-area.space-widget {
		    color: ' . esc_attr ($dark_second_text_color) . ';
		}

		::-webkit-input-placeholder {
		  color: ' . esc_attr ($dark_third_text_color) . ' !important;
		}
		...
		hr.wp-block-separator {
			background-color: ' . esc_attr ($dark_third_text_color) . ';
		}
		...
		$custom_dark_css .= isset( $mercury_dark_css['custom_dark_css'] ) ? $mercury_dark_css['custom_dark_css'] : '';
		wp_add_inline_style( 'mercury-dark', $custom_dark_css );

	}
	
}
add_action( 'wp_enqueue_scripts', 'mercury_dark_mode_css' );
